...
lazy_static! {
    static ref BINARY_NAME: String =
        format!("IpoptBuilder.v{ver}.{suffix}", ver=VERSION, suffix=BINARY_SUFFIX);
    static ref BINARY_URL: String =
        format!("{dl}v{ver}-1-static/{name}",
                dl=BINARY_DL_URL, ver=VERSION, name=BINARY_NAME.as_str());
}

fn main() {
    // Try to find Ipopt preinstalled.
    if let Ok(lib) = pkg_config::Config::new().atleast_version(MIN_VERSION).probe(LIBRARY) {
        dbg!(lib);
        unimplemented!();
    }
...
fn download_and_install_prebuilt_binary() -> Result<(), Error>{
    ...
    // Copy lib
    fs::copy(unpacked_dir.join("lib").join(&library_file), install_dir.join("lib").join(&library_file)).unwrap();
...
fn link(cnlp_install_path: PathBuf) -> Result<(), Error> {
    // Link to cnlp
    println!("cargo:rustc-link-search=native={}", cnlp_install_path.join("lib").display());
    println!("cargo:rustc-link-lib=dylib=ipopt_cnlp");

    // Generate raw bindings to CNLP interface
    let c_api_header = cnlp_install_path
        .join("include")
        .join("c_api.h");
...
fn download_tarball(tarball_path: &Path, binary_url: &str) -> Result<(), Error> {
    if !tarball_path.exists() {
        ...
        if response_code != 200 {
            return Err(Error::DownloadFailure { response_code, url: binary_url.to_string() });
        }
    }
...
fn build_and_install_with_mkl() -> Result<PathBuf, Error> {
    ...
    let blas = {
        if !mkl_root.exists() {
            ...
            let link_libs = format!("-L{mkl}/lib -ltbb -lpthread -lm -ldl", mkl=mkl_root.display());
            if cfg!(target_os = "macos") {
                format!("--with-blas={mkl}intel_lp64.a {mkl}tbb_thread.a {mkl}core.a -lc++ {}",
